...
describe('Інтеграційні тести API Recipe', () => {
    ...
    describe('GET /api/recipes/random', () => {
        it('Повинен повертати випадковий рецепт', async () => {
            await Recipe.create({ title: 'Тест 1', difficulty: 'easy', cookingTimeMinutes: 10, ingredients: ['1'] });
    ...
    describe('PUT /api/recipes/:id', () => {
        it('Повинен оновлювати існуючий документ', async () => {
            ...
            const res = await request(app).put(`/api/recipes/${recipe.id}`).send({
                title: 'Нова назва'
            });
        ...
        it('Повинен повертати 404 для неіснуючого рецепту', async () => {
            const res = await request(app).put('/api/recipes/non-existent-uuid').send({ title: 'Нова назва' });
            expect(res.status).toBe(404);
        });
    });
    ...
    describe('DELETE /api/recipes/:id', () => {
        it('Повинен видаляти існуючий документ', async () => {
            ...
            const dbCheck = await Recipe.findOne({ id: recipe.id });
            expect(dbCheck).toBeNull();
        });

        it('Повинен повертати 404 для неіснуючого рецепту', async () => {
            const res = await request(app).delete('/api/recipes/non-existent-uuid');
            expect(res.status).toBe(404);
        });
    });
    ...
    describe('Перевірка обробки помилок сервера (catch blocks у маршрутах)', () => {
        ...
        it('PUT /api/recipes/:id повинен передавати помилку в next', async () => {
            jest.spyOn(Recipe, 'findOneAndUpdate').mockRejectedValueOnce(new Error('DB Error'));
            const res = await request(app).put('/api/recipes/some-id').send({ title: 'New' });
            expect(res.status).toBe(500);
        });

        it('DELETE /api/recipes/:id повинен передавати помилку в next', async () => {
            jest.spyOn(Recipe, 'findOneAndDelete').mockRejectedValueOnce(new Error('DB Error'));
            const res = await request(app).delete('/api/recipes/some-id');
            expect(res.status).toBe(500);
        });
